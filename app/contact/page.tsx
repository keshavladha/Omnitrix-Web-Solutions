        ...
        <section className="border-t border-slate-900 bg-slate-950/20 py-20">
          <div className="container">
            <div className="lg:grid lg:grid-cols-[1.2fr_1fr] lg:gap-16">
              {/* Contact Form */}
              <Reveal>
                <div className="glass-panel rounded-2xl p-6 lg:p-8 border border-white/5 bg-slate-950/40">
                  ...
                  <form className="space-y-5" action="/api/contact" method="post">
                    ...
                    <div className="grid gap-5 sm:grid-cols-2">
                      <div>
                        ...
                        <input
                           ...
                           className="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-white placeholder:text-slate-400 focus:bg-black/50 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 focus:outline-none transition"
                        />
                      </div>
                      <div>
                        <label className="mb-2 block text-sm font-medium text-slate-400">
                    ...
                    <div className="grid gap-5 sm:grid-cols-2">
                      ...
                      <div>
                        ...
                        <select 
                           ...
                           className="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-white focus:bg-black/50 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 focus:outline-none transition cursor-pointer"
                        >
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-white placeholder:text-slate-400 focus:bg-black/50 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 focus:outline-none resize-none transition"
                      />
                    </div>

                    <button
                      type="submit"
                      className="flex w-full items-center justify-center gap-2 rounded-full bg-cyan-500 px-6 py-3.5 text-sm font-bold text-obsidian-lowest transition hover:bg-cyan-400 shadow-md hover:shadow-cyan-500/20 cursor-pointer"
                      style={{ color: "#02040a" }}
                    >
                    ...
                    <div className="space-y-4">
                      {contactMethods.map((method) => (
                        <a
                          key={method.label}
                          href={method.href}
                          className="flex items-center gap-4 rounded-lg border border-white/5 bg-slate-950/40 p-4 transition hover:border-cyan-500/30 hover:bg-slate-900/40 hover:shadow-sm"
                        >
                  ...
                  <div className="glass-panel rounded-xl p-6 border border-white/5 bg-slate-950/40">
                    ...
                    <ul className="space-y-3 text-sm">
                      ...
                      <li className="flex justify-between">
                        ...
                        <span className="text-white font-semibold">IST (GMT+5:30)</span>
                      </li>
                      <li className="flex justify-between">
                        <span className="text-slate-400 font-medium">Availability</span>
                        <span className="text-cyan-400 font-semibold">● Open for projects</span>
                      </li>
                    </ul>
                  </div>
                </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {officeLocations.map((office, index) => (
              <Reveal key={office.city} delay={0.1 * index}>
                <div className="glass-panel rounded-xl p-6 border border-white/5 bg-slate-950/40">
                  <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg bg-cyan-500/10 border border-cyan-500/20">
                    <MapPin className="h-5 w-5 text-cyan-400" />
                  </div>
                  <h3 className="font-display text-lg font-semibold text-white mb-1">
                    {office.city}
                  </h3>
                  <p className="text-sm text-cyan-400 font-semibold mb-3">{office.country}</p>
